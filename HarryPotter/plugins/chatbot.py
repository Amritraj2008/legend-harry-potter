...
async def szcbot(_, message: Message):
    ...
    if chat:
       ...
       affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Harry&ownername=@amritraj20_08&user=1")
       textmsg = (affiliateplus.json()["message"])
       if "Affiliate+" in textmsg:
        textmsg = textmsg.replace("Affiliate+", "Rose bot created by @tech_sav_bots")
       if "Lebyy_Dev" in textmsg:
        textmsg = textmsg.replace("Lebyy_Dev", "Supun Maduranga Owner of @tech_sav_bots")
